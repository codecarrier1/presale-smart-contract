...
contract PresaleWithSwapping is Ownable {
	...
	mapping(address => uint256) tokensPurchased; //amount of tokens purchased by each address
    mapping(address => bool) whitelist; //whether each address is whitelisted or not
	...
	modifier checkPurchase(address buyer, uint256 amountToBuy) {
		...
		uint256 tokensLeftToSell = tokenForSale.balanceOf(address(this));
		require(amountToSend <= tokensLeftToSell, "amountToSend exceeds contract balance");
        _;
	}
	...
	function _processPurchase(address buyer, uint256 amountToBuy) internal {
        ...
        tokenToPay.safeTransferFrom(buyer, address(this), amountToPay);
        tokenForSale.safeTransfer(buyer, amountToSend);
	}
